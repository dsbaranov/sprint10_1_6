...
struct ColorVisitor {
  std::ostream &os_;
  void operator()(std::monostate) const { os_ << "none"s; }
  void operator()(Rgb rgb) const { os_ << rgb; }
  void operator()(Rgba rgba) const { os_ << rgba; }
  void operator()(std::string color) const { os_ << color; }
};
